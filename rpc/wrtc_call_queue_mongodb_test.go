...
func TestMongoDBWebRTCCallQueueMulti(t *testing.T) {
	...
	t.Run("max queue size", func(t *testing.T) {
		...
		for i := 0; i < maxCallerQueueSize; i++ {
			callID, _, respDone, cancel, err := callerQueue.SendOfferInit(ctx, host, "somesdp", false, "", nil)
			test.That(t, err, test.ShouldBeNil)
		...
		_, _, _, _, err := callerQueue.SendOfferInit(ctx, host, "somesdp", false, "", nil)
		test.That(t, err, test.ShouldResemble, errTooManyConns)
		_, _, _, _, err = answererQueue.SendOfferInit(ctx, host, "somesdp", false, "", nil)
		test.That(t, err, test.ShouldResemble, errTooManyConns)
